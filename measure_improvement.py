...
def pd_rf_to_csv():
    ...
    ef.rf_pd_test(x_dict=x_data, y_dict=y_linear_data, n_tests=5, feats=[0, 1, 2], csv_name='data/rf_pd_test.csv')


def pd_compare_to_csv():
    x_data, y_linear_data, y_non_linear_data = get_data()
    ef.pd_compare_test(x_lst=x_data['x_independent'][:5], y_lst=y_linear_data['x_independent'][:5], feats=[0, 1, 2],
...
def perm_to_csv():
    ...
    ef.permutation_test(x_dict=x_data, y_dict={'linear': y_linear_data, 'non_linear': y_non_linear_data}, feat=1,
...
def perm_rf_to_csv():
    ...
    ef.rf_permutation_test(x_data=x_data['x_no_extrapolation'][0], y_data=y_non_linear_data['x_no_extrapolation'][0],
                           feat=1,
...
def shap_to_csv():
    ...
    ef.shap_kmeans_test(x_dict=x_data, y_dict={'linear': y_linear_data, 'non_linear': y_non_linear_data},
                        feats=[0, 1, 2],
