...
def linear_independent_to_csv():
    ...
    ef.influence_normal_data(x_data=x_data['x_independent'], y_data=y_linear_data['x_independent'], feats=[0, 1, 2],
                             csv_name='data/linear_independent_influence.csv')


def linear_correlated_to_csv():
    x_data, y_linear_data, y_non_linear_data = get_data()
    ef.influence_normal_data(x_data=x_data['x_correlated'], y_data=y_linear_data['x_correlated'], feats=[0, 1, 2],
                             csv_name='data/linear_correlated_influence.csv')


def linear_extrapolation_to_csv():
    x_data, y_linear_data, y_non_linear_data = get_data()
    ef.influence_extrapolation_data(x_no_extrapolation=x_data['x_no_extrapolation'],
...
def non_linear_independent_to_csv():
    x_data, y_linear_data, y_non_linear_data = get_data()
    ef.influence_normal_data(x_data=x_data['x_independent'], y_data=y_non_linear_data['x_independent'],
                             feats=[0, 1, 2], csv_name='data/non_linear_independent_influence.csv')


def non_linear_correlated_to_csv():
    x_data, y_linear_data, y_non_linear_data = get_data()
    ef.influence_normal_data(x_data=x_data['x_correlated'], y_data=y_non_linear_data['x_correlated'],
...
def true_model_influence_to_csv():
    x_data, y_linear_data, y_non_linear_data = get_data()
    ef.influence_true_data(x_dict=x_data, feats=[0, 1, 2], n_simulations=10, csv_name='data/true_model_influence.csv')
